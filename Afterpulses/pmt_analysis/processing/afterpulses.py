...
class AfterPulses:
    """Afterpulse finding and processing with an adjustable early-window ADC cut."""
    ...
    def __init__(
        ...
        adc_f: float,
        verbose: bool = True,
        pre_filter_threshold: float = 3,
        pre_filter_threshold_type: str = "std",
        area_thr_ap: Optional[float] = None,
        t_thr_ap: Optional[float] = None,
        adc_area_to_e: Optional[float] = None,
        gain: Optional[float] = None,
        min_afterpulse_delay_samples: int = 60,
        conditional_delay_min_samples: Optional[int] = 10,
        conditional_delay_max_samples: Optional[int] = 60,
        conditional_min_peak_height_adc: Optional[float] = 100,
    ):
        """
        Args:
            min_afterpulse_delay_samples:
                Global minimum accepted delay between main pulse and afterpulse,
                in samples. If you want to inspect/count candidates from 10 samples
                onward, set this to 10.
            conditional_delay_min_samples:
                Start of the special delay window, in samples.
            conditional_delay_max_samples:
                End of the special delay window, in samples. The cut is inclusive.
            conditional_min_peak_height_adc:
                Minimum p1 peak amplitude in ADC inside the special delay window.
                Set to None to disable the special ADC cut.
        """
        self.adc_f = adc_f
        ...
        self.min_afterpulse_delay_samples = min_afterpulse_delay_samples
        self.conditional_delay_min_samples = conditional_delay_min_samples
        self.conditional_delay_max_samples = conditional_delay_max_samples
        self.conditional_min_peak_height_adc = conditional_min_peak_height_adc
    ...
    def pre_filter(
        self,
        amplitude_threshold_abs: Optional[float] = None,
        amplitude_threshold_std: Optional[float] = 3,
    ):
        if (amplitude_threshold_abs is not None) and (amplitude_threshold_std is not None):
            raise ValueError(
                "Either amplitude_threshold_abs or amplitude_threshold_std must be None "
                "to be disabled, using both options is not permitted."
            )
        if (amplitude_threshold_abs is None) and (amplitude_threshold_std is None):
            raise ValueError("Either amplitude_threshold_abs or amplitude_threshold_std must have a finite value.")

        amplitudes = FullWindow().get_amplitude(self.input_data)
        if amplitude_threshold_abs is not None:
            if self.verbose:
                print(f"Pre-filtering data with absolute threshold {amplitude_threshold_abs}.")
            self.input_data = self.input_data[amplitudes > amplitude_threshold_abs]
        elif amplitude_threshold_std is not None:
            if self.verbose:
                print(f"Pre-filtering data with threshold at {amplitude_threshold_std} baseline standard deviations.")
    ...
    def _passes_conditional_peak_cut(self, delay_samples: int, peak_height_adc: float) -> bool:
        if self.conditional_min_peak_height_adc is None:
            return True
        if self.conditional_delay_min_samples is None or self.conditional_delay_max_samples is None:
            return True

        in_conditional_window = (
            self.conditional_delay_min_samples
            <= delay_samples
            <= self.conditional_delay_max_samples
        )
        if not in_conditional_window:
            return True

        return peak_height_adc >= self.conditional_min_peak_height_adc
    ...
    def find_ap(self, height: float, distance: float = 6, prominence_std: float = 8):
        ...
        if self.input_data.shape[0] > 5e5:
            warnings.warn(
                "Overwriting input_data attribute to save memory, "
        ...
        dt = np.dtype([
            ("idx", np.int32),
            ("input_data_converted", object),
            ("input_data_std", np.float64),
            ("p0_position", np.int32),
            ("p1_position", np.int32),
        ])
        arr = np.zeros(n, dtype=dt)

        idx = 0
        arr_idx = 0
        for i, el in tqdm(enumerate(input_data_converted)):
            ...
            if peak_positions.shape[0] > 1:
                ...
                for p1_position_raw in peak_positions[1:]:
                    p1_position = int(p1_position_raw)
                    delay_samples = p1_position - p0_position
                    p1_peak_height_adc = float(el[p1_position])

                    if delay_samples < self.min_afterpulse_delay_samples:
                        continue

                    if not self._passes_conditional_peak_cut(delay_samples, p1_peak_height_adc):
                        continue
    ...
    def get_ap_properties(self):
        ...
        wf_array = np.array(self.df["input_data_converted"].tolist())
        self.df["p0_amplitude"] = wf_array[np.arange(self.df.shape[0]), np.array(self.df["p0_position"])]
        self.df["p1_amplitude"] = wf_array[np.arange(self.df.shape[0]), np.array(self.df["p1_position"])]

        valley = np.zeros(self.df.shape[0], dtype=int)
        ...
        for i, el in enumerate(self.df["input_data_converted"]):
            input_std = self.df["input_data_std"].iloc[i]
            p0 = self.df["p0_position"].iloc[i]
            p1 = self.df["p1_position"].iloc[i]

            valley[i] = np.argmin(el[p0:p1]) + p0

            try:
                p0_lower_bound[i] = np.where(el[:p0] < input_std)[0][-3]
            except IndexError:
                p0_lower_bound[i] = 0

            try:
                p0_upper_bound[i] = np.where(el[p0:] < input_std)[0][2] + p0
            except IndexError:
                p0_upper_bound[i] = valley[i]
            p0_upper_bound[i] = min(p0_upper_bound[i], valley[i])

            try:
                p1_lower_bound[i] = np.where(el[:p1] < input_std)[0][-3]
            except IndexError:
                p1_lower_bound[i] = valley[i]
            p1_lower_bound[i] = max(p1_lower_bound[i], valley[i])

            try:
                p1_upper_bound[i] = np.where(el[p1:] < input_std)[0][2] + p1
            except IndexError:
                p1_upper_bound[i] = len(el) - 1

            p0_area[i] = np.sum(el[p0_lower_bound[i]:p0_upper_bound[i] + 1])
            p1_area[i] = np.sum(el[p1_lower_bound[i]:p1_upper_bound[i] + 1])
            separable[i] = el[int(valley[i])] <= 0.05 * max(
                self.df.iloc[i]["p0_amplitude"],
        ...
        if (self.adc_area_to_e is not None) and (self.gain is not None):
            self.df["p0_area_pe"] = self.df["p0_area"] * self.adc_area_to_e / self.gain
            self.df["p1_area_pe"] = self.df["p1_area"] * self.adc_area_to_e / self.gain
    ...
    def multi_ap(self):
        ...
        if self.verbose:
            ...
            print(
                "Found total of {} multi-afterpulse candidate waveforms out of {} "
                "({} distinct) general afterpulse candidate waveforms.".format(
                    len(idx_multi_ap),
                    self.df.shape[0],
                    len(np.unique(self.df.idx)),
                )
            )
        ...
        for i, el in enumerate(self.df["input_data_converted"]):
            if (i < n_rows - 1) and (self.df["idx"].iloc[i] == self.df["idx"].iloc[i + 1]):
                if (
                    abs(self.df["p1_lower_bound"].iloc[i] - self.df["p1_lower_bound"].iloc[i + 1]) <= 5
                    and abs(self.df["p1_upper_bound"].iloc[i] - self.df["p1_upper_bound"].iloc[i + 1]) <= 5
                ):
                    valley = np.argmin(el[self.df["p1_position"].iloc[i]:self.df["p1_position"].iloc[i + 1]])
                    valley += self.df["p1_position"].iloc[i]
                    separable = (
                        el[valley] <= 0.3 * self.df["p1_amplitude"].iloc[i]
                        and el[valley] <= 0.3 * self.df["p1_amplitude"].iloc[i + 1]
                    )
                    if separable:
            ...
            if (i > 0) and (self.df["idx"].iloc[i] == self.df["idx"].iloc[i - 1]):
                if (
                    ...
                    valley += self.df["p1_position"].iloc[i - 1]
                    separable = (
                        el[valley] <= 0.3 * self.df["p1_amplitude"].iloc[i - 1]
                        and el[valley] <= 0.3 * self.df["p1_amplitude"].iloc[i]
                    )
                    if separable:
                        mods["index"].append(self.df.index[i])
        ...
        for i, ind in enumerate(mods["index"]):
            ...
            self.df.at[ind, "p1_area"] = np.sum(
                self.df.at[ind, "input_data_converted"][
                    self.df.at[ind, "p1_lower_bound"]:self.df.at[ind, "p1_upper_bound"] + 1
                ]
            )
        ...
        if self.verbose:
            print(
                "Found {} yet unsplit afterpulse candidates and removed {} duplicate waveforms "
                "of merged multi-afterpulse candidates.".format(len(mods["index"]), len(duplicates))
            )
    ...
    def ap_rate(self):
        ...
        mean_p0_area_pe = None
        sum_p0_area_pe = None
        ap_rate_per_pe = None
        ap_rate_per_pe_unc = None
        ap_rate_per_pe_separable = None
        ap_rate_per_pe_separable_unc = None
        n_ap_separable_above_thr = None
        n_ap_separable_below_thr = None
        ap_rate_per_pe_above_thr = None
        ap_rate_per_pe_above_thr_unc = None
        ap_rate_per_pe_below_thr = None
        ap_rate_per_pe_below_thr_unc = None

        if "p0_area_pe" in self.df.columns:
            df_main_unique = self.df.drop_duplicates(subset="idx")
            mean_p0_area_pe = df_main_unique["p0_area_pe"].mean()
            sum_p0_area_pe = df_main_unique["p0_area_pe"].sum()

            if (sum_p0_area_pe is not None) and (sum_p0_area_pe > 0):
                ap_rate_per_pe = n_ap / sum_p0_area_pe
                ap_rate_per_pe_unc = np.sqrt(n_ap) / sum_p0_area_pe
                ap_rate_per_pe_separable = n_ap_separable / sum_p0_area_pe
                ap_rate_per_pe_separable_unc = np.sqrt(n_ap_separable) / sum_p0_area_pe

                if self.area_thr_ap is not None:
                    if "p1_area_pe" in self.df.columns:
                        area_mask = self.df["p1_area_pe"] >= self.area_thr_ap
                    else:
                        warnings.warn("No p1_area_pe found, applying area_thr_ap to raw ADC area.")
                        area_mask = self.df["p1_area"] >= self.area_thr_ap

                    if self.t_thr_ap is None:
                        mask_above = self.df["separable"] & area_mask
                        mask_below = self.df["separable"] & (~area_mask)
                    else:
                        time_mask = self.df["t_diff_ns"] >= self.t_thr_ap
                        mask_above = self.df["separable"] & area_mask & time_mask
                        mask_below = self.df["separable"] & (~area_mask) & time_mask

                    n_ap_separable_above_thr = np.sum(mask_above)
                    n_ap_separable_below_thr = np.sum(mask_below)
                    ap_rate_per_pe_above_thr = n_ap_separable_above_thr / sum_p0_area_pe
                    ap_rate_per_pe_above_thr_unc = np.sqrt(n_ap_separable_above_thr) / sum_p0_area_pe
                    ap_rate_per_pe_below_thr = n_ap_separable_below_thr / sum_p0_area_pe
                    ap_rate_per_pe_below_thr_unc = np.sqrt(n_ap_separable_below_thr) / sum_p0_area_pe

                    if (n_ap_separable_above_thr + n_ap_separable_below_thr) != n_ap_separable:
                        warnings.warn("Above-threshold and below-threshold counts do not add up to total separable count.")

        self.ap_rate_dict = {
            "n_ap": n_ap,
            "n_ap_separable": n_ap_separable,
            ...
            "area_thr_ap": self.area_thr_ap,
            "t_thr_ap": self.t_thr_ap,
            "n_ap_separable_above_thr": n_ap_separable_above_thr,
            "n_ap_separable_below_thr": n_ap_separable_below_thr,
...
#                 input_data_std = self.input_data_std[i]
#                 min_afterpulse_delay = 60   
#                 kept_any = False

#                 for p1_position in peak_positions[1:]:
#                     if p1_position - p0_position < min_afterpulse_delay:
#                         continue
...
#         - `separable`: Boolean stating whether main pulse and afterpulse candidates are separable and not overlapping.
#             Only if true, area and amplitude calculations are reliable.
#         - `p0_area_pe`: Main pulse area in PE, if `adc_area_to_e` and `gain` are defined.
#         - `p1_area_pe`: Afterpulse area in PE, if `adc_area_to_e` and `gain` are defined.
#         """
#         if self.verbose:
#             print('Calculating peak properties.')
...
#             # Position of the lowest value between identified main pulse and afterpulse candidates
#             valley[i] = (
#                 np.argmin(el[self.df['p0_position'].iloc[i]:self.df['p1_position'].iloc[i]])
#                 + self.df['p0_position'].iloc[i]
#             )

#             # Lower window bound for main pulse candidate area estimation
#             try:
#                 p0_lower_bound[i] = np.where(el[:self.df['p0_position'].iloc[i]] < input_std)[0][-3]
#             except IndexError:
#                 p0_lower_bound[i] = 0  # unconstrained

#             # Upper window bound for main pulse candidate area estimation
#             try:
#                 p0_upper_bound[i] = (
#                     np.where(el[self.df['p0_position'].iloc[i]:] < input_std)[0][2]
#                     + self.df['p0_position'].iloc[i]
#                 )
#             except IndexError:
#                 p0_upper_bound[i] = valley[i]
...
#             # Lower window bound for afterpulse candidate area estimation
#             try:
#                 p1_lower_bound[i] = np.where(el[:self.df['p1_position'].iloc[i]] < input_std)[0][-3]
#             except IndexError:
#                 p1_lower_bound[i] = valley[i]
#             p1_lower_bound[i] = max(p1_lower_bound[i], valley[i])

#             # Upper window bound for afterpulse candidate area estimation
#             try:
#                 p1_upper_bound[i] = (
#                     np.where(el[self.df['p1_position'].iloc[i]:] < input_std)[0][2]
#                     + self.df['p1_position'].iloc[i]
#                 )
#             except IndexError:
#                 p1_upper_bound[i] = len(el) - 1
...
#             # Separable if valley is <= 5% of the larger peak
#             separable[i] = (
#                 el[int(valley[i])] <= 0.05 * max(self.df.iloc[i]['p0_amplitude'], self.df.iloc[i]['p1_amplitude'])
#             )
...
#         # Convert areas to PE if calibration is available
#         if (self.adc_area_to_e is not None) and (self.gain is not None):
#             self.df['p0_area_pe'] = self.df['p0_area'] * self.adc_area_to_e / self.gain
#             self.df['p1_area_pe'] = self.df['p1_area'] * self.adc_area_to_e / self.gain
...
#                     if abs(self.df['p1_lower_bound'].iloc[i] - self.df['p1_lower_bound'].iloc[i + 1]) <= 5 and \
#                             abs(self.df['p1_upper_bound'].iloc[i] - self.df['p1_upper_bound'].iloc[i + 1]) <= 5:
...
#     # def ap_rate(self):
#     #     """Calculate afterpulse fractions and afterpulse rates per PE.

#     #     Definitions:
#     #     - Fractions are per trigger / waveform.
#     #     - Rates per PE are normalized by the summed main-pulse PE over unique waveforms.
...
#         - Fractions are per trigger / waveform.
#         - Rates per PE are normalized by the summed main-pulse PE over unique waveforms.
#         - area_thr_ap is interpreted in PE if p1_area_pe exists, otherwise in raw ADC area.
#         - t_thr_ap is optional. If None, no time cut is applied.
...
#                     if (n_ap_separable_above_thr + n_ap_separable_below_thr) != n_ap_separable:
#                         warnings.warn(
#                             "Above-threshold and below-threshold counts do not add up to total separable count."
#                         )
